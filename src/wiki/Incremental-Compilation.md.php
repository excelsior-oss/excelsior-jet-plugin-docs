...
It is implemented via maintaining so called Project Database (PDB).
PDB is a directory created by the JET compiler at the very start of compilation that holds 
all auxiliary files produced by the compiler.
...
that in turn may have one of the following paramters:
...
To clean the PDB for current project, execute the following <?php tool(); ?> command:
...
As a result all the dependencies will be recompiled on a subsequent build.
